...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $assessment_title = trim($_POST['assessment_title'] ?? $assessment['name']);
    ...
    if (isset($_POST['add_question'])) {
        ...
        $correct_answer = $_POST['correct_answer'] ?? 'True';
...
<div class="sidebar">
    ...
    <div class="content">
    <div class="top-bar">
            <h1 class="dashboard-title"> Edit Assessment - True or False</h1>
    ...
    <form method="post">
        ...
        <select name="correct_answer" id="correct_answer" class="styled-select">
    ...
    <?php foreach ($questions as $question): ?>
        <li>
            ...
            <label>Correct Answer:</label>
            <select name="correct_answer-<?php echo $question['question_id']; ?>" id="correct-answer-<?php echo $question['question_id']; ?>" class="styled-select" onchange="updateQuestion('<?php echo $question['question_id']; ?>')"> <option value="True" <?php echo $question['correct_answer'] === 'True' ? 'selected' : ''; ?>>True</option><option value="False" <?php echo $question['correct_answer'] === 'False' ? 'selected' : ''; ?>>False</option>
            </select>
            <br> </br>
...
<script>
    ...
    if (confirm("Are you sure you want to remove this question?")) {
        ...
        try {
            const response = await fetch('assessment_true_false.php?assessment_id=<?php echo $assessment_id; ?>', {
                method: 'POST',
                body: formData
            });
