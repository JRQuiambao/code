...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && (isset($_POST['save_assessment']) || isset($_POST['publish_assessment']) || isset($_POST['unpublish_assessment']) || isset($_POST['update_assessment']))) {
    if (count($questions) === 0) {
        ...
        if ($stmt->rowCount() === 0) {
            ...
            if ($status === 'Published') {
                $_SESSION['message'] = "Assessment published successfully!";
            } elseif ($status === 'Saved' && isset($_POST['unpublish_assessment'])) {
                $_SESSION['message'] = "Assessment unpublished successfully!";
            } elseif (isset($_POST['update_assessment'])) {
                $_SESSION['message'] = "Assessment updated successfully!";
            } else {
                $_SESSION['message'] = "Assessment saved successfully!";
            }
...
<body>
    ...
    <h3>Add Questions</h3>
    <p><strong>Total Points:</strong> <?php echo htmlspecialchars($assessment['total_points'] ?? 1); ?></p>
